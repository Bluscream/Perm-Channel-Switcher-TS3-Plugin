/*
 * PermChanSwitcher :: TeamSpeak 3 Client fun plugin
 *
...
#define PLUGIN_NAME "PermChanSwitcher"
#define PLUGIN_AUTHOR "sk0r / Czybik"
#define PLUGIN_VERSION "0.2"
#define PLUGIN_CONTACT "Czybik_Stylez@gmx.de"

#define PCS_DEFAULT_DELAY 50

int g_iConnectionStatus = STATUS_DISCONNECTED;
...
//Permanent channel switch manager
class CPermChannelSwitcher {
#define INVALID_CLIENTID 0xFFFF
#define INVALID_CHANNELID 0xFFFFFFFFFFFFFFFF
private:
	BOOL m_bStarted;
	DWORD m_dwDelay;
	DWORD m_dwMsCur, m_dwMsLast;
	uint64 m_uiOldChannel;
	anyID m_aidClientID;
	uint64 m_uiServerConnectionHandlerID;

	BOOL IsConnectionEstablished(VOID)
	{
		//Check if a connection to a server is currently established

		int iResult;

		//Query connection status
		if (ts3Functions.getConnectionStatus(m_uiServerConnectionHandlerID, &iResult) != ERROR_ok)
			return FALSE;

		return iResult == STATUS_CONNECTION_ESTABLISHED;
	}

	anyID GetLocalClientID(VOID)
	{
		//Get local client ID

		anyID aidLocalClient;

		if (ts3Functions.getClientID(m_uiServerConnectionHandlerID, &aidLocalClient) != ERROR_ok) {
			return INVALID_CLIENTID;
		}

		return aidLocalClient;
	}

	uint64 GetChannelAmount(uint64* pArray)
	{
		//Get amount of channels

		uint64 uiResult = 0;

		uint64* pPointer = pArray; //Set start address

		while (*pPointer != 0) { //While not at end of list
	...
	uint64 GetCurrentChannel(anyID aidClientId)
	{
		//Get current channel of local client

		uint64 uiResult;

		//Query current channel
		if (ts3Functions.getChannelOfClient(m_uiServerConnectionHandlerID, aidClientId, &uiResult) != ERROR_ok)
			return INVALID_CHANNELID;

		return uiResult;
	}

	BOOL SwitchRandomChannel(VOID)
	{
		//Switch to a pseudo-randomly selected channel

		//Query channel list
		uint64* puiChannelList;
		if (ts3Functions.getChannelList(m_uiServerConnectionHandlerID, &puiChannelList) != ERROR_ok)
			return FALSE;

		//Get channel amount
		uint64 uiChannelAmount = GetChannelAmount(puiChannelList);
		if (uiChannelAmount == 0) {
			ts3Functions.freeMemory(puiChannelList);
			return FALSE;
		}

		//Select a channel number pseudo-randomly
		uint64 uiRandomChannel = puiChannelList[rand()%uiChannelAmount];
		if (uiRandomChannel == GetCurrentChannel(m_aidClientID))
			return TRUE;

		//Request client move
		BOOL bResult = ts3Functions.requestClientMove(m_uiServerConnectionHandlerID, m_aidClientID, uiRandomChannel, "", "CPermChannelSwitcher::SwitchRandomChannel") == ERROR_ok;
...
public:
	CPermChannelSwitcher() { m_bStarted = FALSE; m_uiOldChannel = INVALID_CHANNELID; m_aidClientID = INVALID_CLIENTID; }
	~CPermChannelSwitcher() { }

	BOOL GetActivationStatus(VOID)
	{
	...
	VOID SetServerConnectionHandlerID(uint64 uiServerConnectionHandlerID)
	{
		//Set current connection handler ID

		m_uiServerConnectionHandlerID = uiServerConnectionHandlerID;
	}

	BOOL SetActivation(BOOL bStatus, anyID aidClientID = INVALID_CLIENTID, DWORD dwDelay = PCS_DEFAULT_DELAY)
	{
		//Set activation status
		
		if ((m_bStarted) && (bStatus)) { //Only one channel switch process is currenlty supported
			ts3Functions.printMessageToCurrentTab("[" PLUGIN_NAME "] Only a single user can be handled at the same time currently"); 
			return FALSE;
		} else if ((!m_bStarted) && (!bStatus)) { //Nothing to change
			return TRUE;
		}
		...
		if (m_bStarted) { //Activate
			if (!IsConnectionEstablished()) {
				//We need an active connection if the process shall be initialized
				m_bStarted = FALSE;
				return FALSE;
			}

			//Get local client ID if no remote client ID is specified

			m_aidClientID = aidClientID;

			if (m_aidClientID == INVALID_CLIENTID) { 
				m_aidClientID = GetLocalClientID();
			...
			m_uiOldChannel = GetCurrentChannel(m_aidClientID);

			m_dwMsCur = m_dwMsLast = GetTickCount(); //Initialize timers
		} else { //Deactivate
			//Attempt to switch back to old channel
			if (m_uiOldChannel != INVALID_CHANNELID) {
				Sleep(200);

				ts3Functions.requestClientMove(m_uiServerConnectionHandlerID, m_aidClientID, m_uiOldChannel, "", "CPermChannelSwitcher::SetActivation");
				
				m_uiOldChannel = INVALID_CHANNELID;
			}
		}
	...
	VOID OnClientDisconnect(uint64 uiServerConnectionHandlerID, anyID aidClientID)
	{
		//Should be called when a client timeout occured. Deactivate action if the client is our target client

		if ((uiServerConnectionHandlerID != m_uiServerConnectionHandlerID) || (m_aidClientID != aidClientID))
			return;

		ts3Functions.printMessageToCurrentTab("[" PLUGIN_NAME "] Client disconnected, stopping action."); 
	...
	VOID Process(VOID)
	{
		...
		if (!IsConnectionEstablished()) {
			ts3Functions.printMessageToCurrentTab("[" PLUGIN_NAME "] Connection to server lost, stopping action."); 
		...
		m_dwMsCur = GetTickCount(); //Update timer with current ms
		if (m_dwMsCur > m_dwMsLast + m_dwDelay) { //Check if determined time has passed
			m_dwMsLast = GetTickCount(); //Update with current ms

			if (!SwitchRandomChannel()) { //Attempt to switch to a pseudo-randomly selected channel
				ts3Functions.printMessageToCurrentTab("[" PLUGIN_NAME "] Failed to switch user, stopping action."); 
...
CPermChannelSwitcher g_PermChanSwitcher;

HANDLE g_hCsThread;
HANDLE g_hEvent;

DWORD WINAPI thChannelSwitcher(LPVOID lpvArgument)
{
	//Thread for perm channel switcher
	
	bool bShallRun = true;

	while (bShallRun) {
		g_PermChanSwitcher.Process();

		//Wait for event being raised
...
VOID LaunchSwitchProcess(anyID aidClientId = INVALID_CLIENTID)
{
	//Launch switch process

	static char szLocalInfo[] = "Warning: You might get kicked/banned because of flooding the server!\n\nAre you sure you want to continue?";
	static char szRemoteInfo[] = "Warning: The specified user might get kicked/banned because of flooding the server!\n\nNote: You need administrative rights to perform this action.\n\nAre you sure you want to continue?";

	//Check if connection has been established
	if (g_iConnectionStatus != STATUS_CONNECTION_ESTABLISHED) {
		MessageBoxA(0, "You must have joined a server in order to launch a switch process.", "Error", MB_ICONERROR);
		return;
	}

	//Check if already activated
	if (g_PermChanSwitcher.GetActivationStatus()) {
		MessageBoxA(0, "Currently the switch process can only be used for a single user, not multiple users. Please stop the current switch process first.", "Error", MB_ICONERROR);
		return;
	}

	//Confirm this action
	int iMsgBoxResult = MessageBoxA(0, (aidClientId == INVALID_CLIENTID) ? szLocalInfo : szRemoteInfo, "Confirm action", MB_YESNO | MB_ICONQUESTION);
	if (iMsgBoxResult == IDNO)
		return;

	//Launch action
	(g_PermChanSwitcher.SetActivation(TRUE, aidClientId)) ? ts3Functions.printMessageToCurrentTab("[" PLUGIN_NAME "] Activated!") : ts3Functions.printMessageToCurrentTab("[" PLUGIN_NAME "] Activation failed!");
}

VOID StopSwitchProcess(VOID)
{
	//Stop switch process

	if (!g_PermChanSwitcher.SetActivation(FALSE)) {
		ts3Functions.printMessageToCurrentTab("[" PLUGIN_NAME "] Deactivation failed!");
		MessageBoxA(0, "Deactivation failed! Terminating program", "Fatal error", MB_ICONERROR);
...
const char* ts3plugin_description() {
	/* If you want to use wchar_t, see ts3plugin_name() on how to use */
    return "Permanent Channel Switcher (" PLUGIN_NAME ")\n\n"
		"Purpose:\n"
		"Attempt to switch to any new pseudo-randomly selected channel permanently in a very short time (~50ms) until the user initiates to stop the process.\n"
		"This works both for the local client and also remote users. Note that in order to use this with remote users, you need to have administrative server rights.\n\n"
		"Warning: Use this only if you are server administrator, otherwise there is a risk on being kicked/banned. Currently the switch process can only be used\n"
		"for a single user at the same time.\n\n"
...
int ts3plugin_init() {
	...
	//Create event for thread safety issues
	...
	//Create channel switcher related thread
	g_hCsThread = CreateThread(NULL, 0, &thChannelSwitcher, NULL, 0, NULL);
	if (!g_hCsThread) {
		printf("CreateThread() failed for \'thChannelSwitcher\': %d\n", GetLastError());
		return 1;
	}
...
void ts3plugin_shutdown() {
	...
	//Raise signal to let the thread function end
	if (SetEvent(g_hEvent)) {
		//Wait until thread execution has finished
		if (WaitForSingleObject(g_hCsThread, INFINITE) == WAIT_OBJECT_0) {
			//Perform stuff here
		}
	}

	//Close thread handle
	if (g_hCsThread != INVALID_HANDLE_VALUE)
		CloseHandle(g_hCsThread);
...
void ts3plugin_initMenus(struct PluginMenuItem*** menuItems, char** menuIcon) {
	...
	CREATE_MENU_ITEM(PLUGIN_MENU_TYPE_GLOBAL,  MENU_ID_GLOBAL_1,  "Start",  "start.png");
	CREATE_MENU_ITEM(PLUGIN_MENU_TYPE_GLOBAL,  MENU_ID_GLOBAL_2,  "Stop",  "stop.png");
	CREATE_MENU_ITEM(PLUGIN_MENU_TYPE_GLOBAL, MENU_ID_GLOBAL_3, "About", "about.png");
	CREATE_MENU_ITEM(PLUGIN_MENU_TYPE_CLIENT, MENU_ID_CLIENT_1, "Start", "start.png");
	CREATE_MENU_ITEM(PLUGIN_MENU_TYPE_CLIENT, MENU_ID_CLIENT_2, "Stop", "stop.png");
...
void ts3plugin_onClientMoveEvent(uint64 serverConnectionHandlerID, anyID clientID, uint64 oldChannelID, uint64 newChannelID, int visibility, const char* moveMessage) {
	if (visibility == LEAVE_VISIBILITY)
		g_PermChanSwitcher.OnClientDisconnect(serverConnectionHandlerID, clientID);
}
...
void ts3plugin_onMenuItemEvent(uint64 serverConnectionHandlerID, enum PluginMenuType type, int menuItemID, uint64 selectedItemID) {
	...
	g_PermChanSwitcher.SetServerConnectionHandlerID(serverConnectionHandlerID); //Set server connection handler ID
		...
		case PLUGIN_MENU_TYPE_GLOBAL:
			...
			switch(menuItemID) {
				case MENU_ID_GLOBAL_1: {
					...
					/* Show about message */
					MessageBoxA(0, PLUGIN_NAME " v" PLUGIN_VERSION " developed by " PLUGIN_AUTHOR " (" PLUGIN_CONTACT ")", "About", MB_ICONINFORMATION);
					break;
			}default: {
					break;
			}
		...
		case PLUGIN_MENU_TYPE_CHANNEL:
			...
			switch(menuItemID) {
				case MENU_ID_CHANNEL_1:
					/* Purpose: Initiate switch process */
					break;
		...
		case PLUGIN_MENU_TYPE_CLIENT:
			...
			switch(menuItemID) {
				case MENU_ID_CLIENT_1: {
					...
					LaunchSwitchProcess((anyID)selectedItemID);
